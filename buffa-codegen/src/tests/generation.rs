...
fn test_empty_file() {
    ...
    // Five sibling files per proto: owned `.rs`, view `.__view.rs`,
    // ext `.__ext.rs`, oneofs `.__oneofs.rs`, view_oneofs
    // `.__view_oneofs.rs`. The ancillary siblings are empty-bodied
    // when the proto has no relevant content, but they're still
    // produced so generate_module_tree can emit stable `include!`
    // paths without probing the filesystem.
    assert_eq!(files.len(), 5);
    let names: Vec<&str> = files.iter().map(|f| f.name.as_str()).collect();
    assert!(names.contains(&"empty.rs"));
    assert!(names.contains(&"empty.__view.rs"));
    assert!(names.contains(&"empty.__ext.rs"));
    assert!(names.contains(&"empty.__oneofs.rs"));
    assert!(names.contains(&"empty.__view_oneofs.rs"));
    for f in &files {
        assert!(
            f.content.contains("@generated by protoc-gen-buffa"),
            "missing header comment on {}",
            f.name
        );
    }
}
...
fn test_message_oneof() {
    ...
    // `pub mod with_oneof` is the owner sub-module for nested messages;
    // it's now absent because this message has no nested types. The
    // oneof enum lives in the `oneofs::` tree, which provides its own
    // `pub mod with_oneof` wrapper inside `pub mod oneofs { … }` — we
    // only check for the enum itself.
    assert!(
