...
fn singular_to_owned(
    ...
    Ok(match ty {
        ...
        // Lazy: deferred validation surfaces through the fallible
        // to_owned_* signature — malformed deferred bytes propagate as the
        // DecodeError that `get()` would have reported.
        Type::TYPE_MESSAGE if ctx.config.lazy_views => {
...
fn repeated_to_owned(
    ...
    match ty {
        ...
        // Lazy: malformed deferred bytes propagate — see `singular_to_owned`.
        Type::TYPE_MESSAGE if ctx.config.lazy_views => {
            quote! {
                self.#ident
                    .iter()
                    .map(|r| r.and_then(|v| v.to_owned_from_source(__buffa_src)))
