...
///
/// Buffa's generated-code layout places ancillary types in
/// **kind-namespaced parallel trees** under a single reserved
/// `__buffa::` parent at each package root (see DESIGN.md → "Generated
/// code layout"). The `__buffa` prefix is reserved by codegen
/// validation, so the kind trees can never collide with proto-derived
/// module names like `message View {}` or `package foo.view`. For kinds
/// that modify other kinds (only `view` today), the modifier wraps the
/// modified:
///
/// - `pkg::<ident>` — owned types at package scope (messages, nested
///   modules, package-level enums).
/// - `pkg::__buffa::view::<proto-path>::<Ident>View` — views mirroring
///   the owned proto-message path. Views retain the `View` suffix (the
///   one documented exception to the tree-disambiguates-ident rule —
///   message + view are commonly co-imported into the same scope).
/// - `pkg::__buffa::oneofs::<proto-path>::<Ident>` — oneof enums,
///   lifted out of the owned tree into a dedicated parallel tree.
/// - `pkg::__buffa::view::oneofs::<proto-path>::<Ident>` — views of
///   oneof enums (the `view` modifier wraps the `oneofs` kind).
/// - `pkg::__buffa::ext::<Ident>` — file-level extension consts +
///   `register_types`.
///
...
///   and nested-view sub-modules), destined for `__buffa::view`.
/// - `<stem>.__ext.rs` — ext tree contents (extension consts + register
///   fn), destined for `__buffa::ext`.
/// - `<stem>.__oneofs.rs` — owned oneof enums grouped by owner,
///   destined for `__buffa::oneofs`.
...
pub fn generate_module_tree(
    ...
    fn emit(out: &mut String, node: &ModNode, depth: usize, prefix: &str, lints: &str) {
        ...
        // single `pub mod __buffa { ... }` wrapper so they cannot
        // collide with proto-derived module names (e.g. a user
        // `message View { ... }` or `package foo.view;`). The `__buffa`
        // prefix is reserved by codegen validation.
        let emit_view_wrapper = !node.view_files.is_empty() || !node.view_oneofs_files.is_empty();
        let emit_buffa =
            emit_view_wrapper || !node.ext_files.is_empty() || !node.oneofs_files.is_empty();
        if emit_buffa {
            ...
            if emit_view_wrapper {
                ...
                if !node.view_oneofs_files.is_empty() {
                    ...
                    for file in &node.view_oneofs_files {
                        writeln!(out, r#"{indent}            include!("{prefix}{file}");"#)
                            .unwrap();
                    }
...
fn check_reserved_field_names(file: &FileDescriptorProto) -> Result<(), CodeGenError> {
    ...
    // A `package foo.__buffa;` segment would emit a `pub mod __buffa { ... }`
    // sibling to the kind-tree wrapper at the parent scope. The `__buffa`
    // prefix is reserved.
    for seg in package.split('.') {
        if !seg.is_empty() && crate::oneof::to_snake_case(seg).starts_with("__buffa") {
            return Err(CodeGenError::ReservedName {
                scope: format!("package {package}"),
                name: seg.to_string(),
            });
        }
    }
...
fn check_module_name_conflicts(file: &FileDescriptorProto) -> Result<(), CodeGenError> {
    ...
    // The `__buffa` module is the single reserved name claimed at every
    // package and nested-message scope for the ancillary kind trees
    // (view / oneofs / ext). Any proto-derived type name that snake-cases
    // to a `__buffa` prefix would collide with that wrapper.
    fn check_reserved(name: &str, scope: &str) -> Result<(), CodeGenError> {
        if crate::oneof::to_snake_case(name).starts_with("__buffa") {
            return Err(CodeGenError::ReservedName {
    ...
    fn check_siblings(
        ...
        enums: &[crate::generated::descriptor::EnumDescriptorProto],
        scope: &str,
    ) -> Result<(), CodeGenError> {
        ...
        for en in enums {
            check_reserved(en.name.as_deref().unwrap_or(""), scope)?;
        }

        for msg in messages {
            let name = msg.name.as_deref().unwrap_or("");
            let module_name = crate::oneof::to_snake_case(name);
            check_reserved(name, scope)?;
            ...
            check_siblings(&msg.nested_type, &msg.enum_type, &child_scope)?;
        }
    ...
    let package = file.package.as_deref().unwrap_or("");
    check_siblings(&file.message_type, &file.enum_type, package)
}
...
fn generate_file(
    ...
    // `pub mod __buffa { pub mod ext { ... } }` at the package level
    // (two hops below package root), so type references inside them
    // use nesting=2.
    ...
    // form and need `super::super::` to climb out of `__buffa::ext::` to
    // package level.
    ...
    let has_ext_content = !reg.is_empty()
        || !file_ext_tokens.is_empty()
        || !file_ext_json_refs.is_empty()
        || !file_ext_text_refs.is_empty();
    if has_ext_content {
        let json_any: Vec<TokenStream> = reg
            .json_any
            .iter()
            .map(|p| quote! { super::super::#p })
            .collect();
        let text_any: Vec<TokenStream> = reg
            .text_any
            .iter()
            .map(|p| quote! { super::super::#p })
            .collect();
        // Nested-in-message ext consts: climb out of `__buffa::ext::` to
        // package scope.
        let nested_json_ext: Vec<TokenStream> = reg
            .json_ext
            .iter()
            .map(|p| quote! { super::super::#p })
            .collect();
        let nested_text_ext: Vec<TokenStream> = reg
            .text_ext
            .iter()
            .map(|p| quote! { super::super::#p })
            .collect();
        ...
        let register_fn = if ctx.config.emit_register_fn && has_any_reg {
            quote! {
...
pub enum CodeGenError {
    ...
    /// A proto message or enum name snake-cases to a `__buffa`-prefixed
    /// module name, which would collide with the ancillary kind-tree
    /// wrapper buffa emits at every package and nested-message scope.
    #[error(
        "reserved name '{name}' in '{scope}': proto names that snake-case \
             to a '__buffa' prefix conflict with buffa's ancillary module"
    )]
    ReservedName { scope: String, name: String },
