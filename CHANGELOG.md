...
- **Module redefinition error when a message and a sub-package share a name
  ...
  deconflicts the **nested-types module** by appending `_` (e.g. `oof_`),
  ...
  previously failed to compile, so existing output is unchanged. Note: if you
  *add* a sub-package whose name collides with an existing message's
