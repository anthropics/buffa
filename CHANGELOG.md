...
- **Opt-in lazy views** (#165). `Config::lazy_views(true)` makes generated
  ...
  budget and unknown-field allowance recorded at decode time are charged on
  access, so lazy chains stay depth- and allowance-bounded exactly like the
  eager decoder (and `DecodeOptions` recursion and unknown-field limits flow
  through); re-encoding replays the recorded fragments verbatim. Malformed
  deferred bytes surface on access, and propagate as `DecodeError` through
  the fallible `to_owned_message`. `get_or_default()` mirrors the eager
...
- **`ViewMerge` trait**: every generated view now exposes its proto merge
  machinery generically (`merge_view(&mut self, buf, ctx)`, taking the same
  `DecodeContext<'_>` as the decode path), used by lazy views to reassemble
  fields split across wire occurrences. Additive for existing code.
