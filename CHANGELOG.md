...
- **Warn at generation time when a kept file references a type from an excluded or unmapped package** (#352, closes #292).
...
- **`CodeGenConfig::shared_descriptor_pool_root`** (#399): override the computed `super::`-relative path to the shared `__buffa_fds` root in shared-pool mode (`shared_descriptor_pool`) with a caller-supplied path, for workspaces where one crate does not host the whole package tree as nested modules (e.g. one independent Cargo crate per proto package). `None` (the default) keeps the existing `super::`-relative behavior — purely additive, no change to any existing consumer. Reachable only by direct `buffa-codegen` callers this release; `buffa-build` and the `protoc-gen-buffa*` plugins do not expose it yet.
...
- **Corrected two decode-limit documentation claims that measurement contradicts** (#349). The unknown-field count limit was described as capping overhead at roughly `limit x 40` bytes; that prices a flat field, and each unknown group additionally allocates a `Vec` whose minimum non-zero capacity is four elements, so a payload of many shallow groups reaches about five times that. And the `Map` hasher docs described the foldhash process-wide seed as mixing ASLR addresses with process start time; that holds on `std`, but `no_std` builds drop the clock and allocator entropy, and on a bare-metal target with no ASLR the seed is a link-time constant, so a colliding key set can be precomputed from the firmware image. Both are documentation-only; no behaviour changes.
...
- **Extension registry lookups by `(extendee, number)` no longer allocate** (#393) in either the JSON or the text registry. Internal representation only; no API, wire, or behaviour change.
...
- **JSON `null` on a registered extension key unsets the extension** (#355), for both singular and repeated extensions. Singular `google.protobuf.Value` and `google.protobuf.NullValue` extensions are the exception: `null` is a present value there. A `NullValue` extension now also serialises as `null` rather than the enum name `"NULL_VALUE"` (`[null, …]` when repeated), matching the reflective codec and ProtoJSON; the parse side accepts `null`, `"NULL_VALUE"`, and `0`. Generated extension registrations reach that behaviour through four new `extension_registry::helpers` functions (`null_value_to_json`, `null_value_from_json`, and their `repeated_` forms), so code generated by this version needs a matching `buffa`.

- **Integer extensions now use the same ProtoJSON decoders as generated fields** (#356), accepting exact quoted decimal and exponent forms across every integer wire type while rejecting unsafe high-magnitude unquoted floats.
