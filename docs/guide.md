...
- **A custom string used as a `map` key needs `Hash + Eq`** (for the default / `HashMap` container) or `Ord` (for `map_type(BTreeMap)`). The bound is enforced at the generated map field type, so a missing impl is a clear compile error at that field. Under `generate_reflection_vtable` such a key also needs `Ord` regardless of container, because codegen emits a `ReflectMapKey` impl for it and that trait requires `Ord` — the reflective map deduplication sorts rather than hashes, so that attacker-controlled keys cannot be made to collide.
