...
By default every generated message contains its own size, write, and merge code, specialised to its fields. `CodecStrategy::Table` replaces it with one static table per message and interpreters in `buffa` that every message shares. On a schema of 334 top-level messages and 3,477 fields, the code section of a binary built at `opt-level = "z"` went from 1,670 KB to 1,010 KB, with the 43 messages that have a `oneof` or a `map` still unrolled. The same schema with its oneofs and maps flattened, so that every message can use the table, went from 1,644 KB to 817 KB (see [#463](https://github.com/anthropics/buffa/issues/463) for the method). The cost is speed on messages made of many small fields, where encoding takes up to about 3.5 times as long as with the default `CodecStrategy::Unrolled` and decoding up to 1.6 times. Messages dominated by bulk data, such as large strings, bytes, and packed arrays, show no difference.
...
A table message may hold any message. It reaches a child that has a table through it, and any other child (a message you set to `Unrolled`, one generated by another crate, or a well-known type such as `Timestamp`) through its `Message` impl, which costs a function call per child. A `codec_strategy_in` rule selects the message it names and the messages nested in it, and does not extend to the messages it holds. In the example, `.wa.Message` stays specialised and the messages that hold it use the table.

The option changes only the binary `Message` implementation. The wire format and the JSON, text, view, and reflection code are the same under both strategies, and a table message encodes to the same bytes and decodes the same accepted input as its unrolled twin. It differs in four ways:
...
- A child that has no table, encoded into a sink that is not a `BufMut`, such as `Rope`, is staged in a buffer first, so the `bytes` fields inside it are copied and not shared by reference count.
