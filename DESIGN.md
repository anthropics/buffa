...
- **A oneof is one table entry per member.** The `Option<Enum>` that holds a oneof has no specified layout, so the entry of a member cannot name a field of the message by offset. Every member has an entry of kind `OneofLeader` (the lowest-numbered member) or `OneofFollower` at the offset of the `Option`, and generated code implements `buffa::table::OneofEnum` for the enum: the number of the member that is set, a pointer to its value, and a way to set a member by number. That trait is safe to implement, and its methods check the type of each value at compile time, so the `unsafe` stays in `buffa` as it does for other fields. A member's value is read and written by the arm of its payload kind, the same arm an ordinary field of that type uses, so a oneof adds no per-type code to the interpreters. The leader writes and sizes the whole oneof at its own place among the fields, as unrolled code does, so the bytes are the same, and the followers only decode; a follower costs one jump in the size and write loops, without a call. Decoding a message member merges into the value if that member is already set and starts from a default otherwise, and a closed enum reads the number before it replaces the member, so an unknown number goes to the unknown fields and leaves the member alone. One difference from unrolled code remains: if a message member that replaces another fails to decode, the oneof is left holding the part that was read.
