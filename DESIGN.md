...
**The contract is a blanket marker trait, not a hand-written impl.** `ProtoString` and `ProtoBytes` (in `buffa`) are bounded entirely on standard-library traits — `Clone + PartialEq + Default + Debug + Send + Sync`, plus `Deref<Target = str>` + `AsRef<str>` + `From<String>` + `From<&str>` for strings, and `Deref<Target = [u8]>` + `AsRef<[u8]>` + `From<Vec<u8>>` for bytes — and each carries a blanket `impl<T: ...> Proto{String,Bytes} for T`. The `Deref` bound is load-bearing rather than cosmetic: generated code borrows a field as `&str` / `&[u8]` by plain reference coercion (`&self.field`), so a representation must `Deref` to the slice type, not merely offer `AsRef`. So a downstream user never names or implements these traits: any type that already satisfies the bounds qualifies automatically. (`ProtoBytes` deliberately omits `From<&[u8]>`, because `bytes::Bytes` implements that only for `&'static [u8]`; requiring it would exclude `Bytes` itself.) buffa carries no per-type knowledge of any specific representation — the only thing baked into codegen is the *default* fast paths.

**Construction goes through standard conversion traits, and the defaults keep their specialized paths.** The decode path constructs the field with `decode_string_to::<S>()` / `decode_bytes_to::<B>()`, which read and validate exactly as the non-generic primitives do and then convert via `From`. The two default representations keep their hand-tuned paths: `String` reuses its buffer in place via `merge_string`, `Vec<u8>` via `merge_bytes`, and `bytes::Bytes` decodes zero-copy via `decode_bytes_to_bytes`. Every other representation takes the generic path, which for the default types is the identity conversion and therefore free. Clearing a field resets it to `Default::default()` rather than calling `.clear()`, because a substituted type may be immutable. The `arbitrary` support is likewise type-agnostic: codegen attaches a generic builder (`arbitrary_proto_string::<S>` / `arbitrary_proto_bytes::<B>`) that materializes the canonical `String` / `Vec<u8>` and converts, so a substituted type needs no native `Arbitrary` impl and codegen needs no knowledge of any specific type.

**Exotic types reach the seam by newtype.** Because the bound is on std traits, the common alternatives (`SmolStr`, `Bytes`, and friends) plug in with zero glue. A type that does not satisfy the bound — or that needs behavior buffa's blanket path cannot express, such as zeroize-on-drop — wraps in a local newtype that implements the missing pieces, the same escape hatch `extern_path` uses for whole-message custom types. There is one reflection-specific caveat: a `repeated <custom>` field reflected in vtable mode needs `<custom>: ReflectElement`, which has no blanket impl (it would collide with the concrete scalar impls under coherence, see §11), so codegen emits the impl for the custom type — and that only compiles when the type is **local** to the generating crate. A foreign type used in that exact combination — repeated, custom representation, *and* vtable reflection — must therefore be newtyped; every other use (singular fields, bridge-mode reflection, non-reflective use) works with the foreign type directly.
