...
- **A child reaches the interpreters through its table or its `Message` impl.** A message field's `Aux` descriptor holds either the child's `MessageTable`, which the interpreters recurse into, or a `DynVt` of three function pointers instantiated for the child type, `compute_size`, `write_to::<PreSized>`, and `merge_length_delimited`, which call the child's `Message` impl (`table/bridge.rs`). The second form serves an unrolled child, a child from another crate, and a well-known type, so a message can use the table whatever it holds. A function pointer cannot be generic over the sink, so the write thunk takes the `PreSized` cursor that every `BufMut` is written through, and any other sink receives the child from a scratch buffer, which costs one copy. The planner in `buffa-codegen` (`table_plan.rs`) selects the messages that asked for the table, drops those the interpreters cannot handle (oneofs, maps, groups and the types of group fields, `MessageSet`, extension ranges with JSON, custom string, bytes, or collection types), and the emitter names a child's static table only if the plan gave the child one.
