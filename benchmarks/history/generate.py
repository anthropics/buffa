...
def render_report(runs: list[dict], matrix: dict[tuple[str, str], dict[str, float]]) -> str:
    ...
    w("")
    w("Throughput of buffa's own protobuf benchmarks, measured once per release on")
    w("a dedicated bare-metal box (turbo off, `performance` governor, core-pinned).")
    w("Each release was built from its own tag and run as it shipped, so the numbers")
    w("are what that release actually delivered. The headline metric is **throughput")
    w("in MiB/s**, which stays comparable across releases even when a tag's benchmark")
