fn main() {
    ...
    // prefix; views + JSON + text on so all cross-reference emission paths
    // compile against the prefixed names. Module names, oneof enums, and
    // the wire format are unaffected. Runtime checks in tests/type_prefix.rs.
    let prefix_out =
