...
fn person_roundtrip() {
    ...
    p.maybe_age = Some(30);
    p.contact = Some(crate::basic::oneofs::person::Contact::Email(
...
fn oneof_variants() {
    let mut p = Person::default();
    p.contact = Some(crate::basic::oneofs::person::Contact::Phone(
        "555-1234".into(),
    ));
    ...
    assert_eq!(
        p.contact,
        Some(crate::basic::oneofs::person::Contact::Phone("b".into()))
    );
}
...
fn group_in_oneof_uses_type_name() {
    use crate::proto2::oneofs::view_coverage::Choice;
