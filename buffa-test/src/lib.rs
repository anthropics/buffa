...
/// Strict JSON unknown-field rejection (#444) — `deny_unknown_json_fields_in`
/// over the derive path, the hand-written-visitor path and the extension path,
/// with lenient siblings in the same module as the control.
#[allow(clippy::derivable_impls, clippy::match_single_binding)]
pub mod strictjson {
    buffa::include_proto!("buffa.test.strictjson");
}

/// Strict JSON unknown-field rejection (#444), proto3 field shapes — the
/// mixed-shape message that checks nothing drops out of the accepted-key list.
#[allow(clippy::derivable_impls, clippy::match_single_binding)]
pub mod strictjson3 {
