...
//! write every `BufMut` through. Any other sink (a [`Rope`](crate::Rope), a
//! sink defined outside this crate, or a `BufMut` passed straight to
//! `Message::write_to`) gets the child from a scratch `Vec` allocated for it,
//! once per element of a repeated field, and the scratch is copied to the
//! sink. A `Rope` therefore copies the child a second time into its tail, and
//! cannot share the `bytes` fields inside it by reference count.
//!
...
//! the whole input. Every `bytes::Bytes` field of the child is copied out of
//! that slice, where unrolled code decoding from a `Bytes` shares it with the
//! input. Code generation keeps the holders of such a message of its own run
//! unrolled, but it cannot see into another crate, so a child from another
//! crate that has a `Bytes` field, such as `google.protobuf.Any`, is copied.
...
/// impl. Built by [`MsgVt::new_via_message`](super::MsgVt::new_via_message) and
/// [`RepVt::new_via_message`](super::RepVt::new_via_message).
