...
//! - A child message that has no table, because it is unrolled, generated by
//!   another crate or a well-known type, is reached through its
//!   [`Message`](crate::Message) impl (see [`MsgVt::new_dyn`]), so it decodes
//!   and encodes exactly as it does inside an unrolled parent. Encoding it into
//!   a sink that is not a [`BufMut`](crate::bytes::BufMut) stages its bytes in a
//!   buffer, which copies them.
//!
